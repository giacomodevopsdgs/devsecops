...
app = Flask(__name__)
CORS(app, resources={r"/api/*": {"origins": "*", "methods": ["GET", "POST", "PUT", "DELETE"], "allow_headers": "*", "supports_credentials": True}})
...
@app.route('/nuovoendpoint', methods=['GET'])
def nuovoendpoint():
  return jsonify({'message': 'The server is running'})
# create a user
@app.route('/nuovoendpoint2', methods=['GET'])
def nuovoendpoint2():
  return jsonify({'message': 'The server is running'})
# create a user
